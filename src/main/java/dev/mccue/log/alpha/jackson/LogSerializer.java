...
public final class LogSerializer extends JsonSerializer<Log> {
    ...
    public void serialize(
        ...
        jsonGenerator.writeStringField("log.alpha/trace-id", log.flake().toString());
        switch (log.context()) {
            case Log.Context.Global __ ->
                jsonGenerator.writeStringField("log.alpha/root-trace", log.flake().toString());
            case Log.Context.Child.Span span -> {
                jsonGenerator.writeStringField("log.alpha/parent-trace", span.spanId().toString());
                var rootSpan = span.rootSpan().orElse(null);
                if (rootSpan != null) {
                    jsonGenerator.writeStringField("log.alpha/root-trace", rootSpan.spanId().toString());
                }
                var parentSpan = span.parentSpan().orElse(null);
        ...
        if (log instanceof Log.Span span) {
            switch (span.outcome()) {
                case Log.Span.Outcome.Ok __ ->
                    jsonGenerator.writeStringField("log.alpha/outcome", "ok");
                case Log.Span.Outcome.Error error -> {
                    jsonGenerator.writeStringField("log.alpha/outcome", "error");
                    jsonGenerator.writeStringField("exception", throwableToString(error.throwable()));
                }
            }
        }
